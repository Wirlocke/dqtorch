...
class _Quaternion_mul_backward(Function):
    ...
    def forward(ctx, grad, inputs_1, inputs_2):
        B, R1, R2, D1, D2, B1, B2 = _get_broadcast_meta_data(
            inputs_1, inputs_2)
        ...
        _backend.quaternion_mul_backward(grad, B, R1, R2, D1, D2, inputs_1, inputs_2, grad_inputs_1, grad_inputs_2)  # type: ignore # nopep8
    ...
    def backward(ctx, *grad_outputs):
        ...
        _backend.quaternion_mul_backward_backward(grad_out_1, grad_out_2,  # type: ignore
                                                  B, R1, R2, D1, D2,
...
class _Quaternion_mul(Function):
    ...
    def forward(ctx, inputs_1: torch.Tensor, inputs_2: torch.Tensor):
        ...
        _backend.quaternion_mul_forward(inputs_1, inputs_2, outputs, B, R1, R2, D1, D2)  # type: ignore # nopep8
    ...
    @staticmethod
    @custom_bwd(device_type='cuda')
    def backward(ctx, grad):  # type: ignore
        # grad: [B, C * C]
        ...
        grad_inputs_1, grad_inputs_2 = _quaternion_mul_backward(
...
class _Quaternion_conjugate(torch.autograd.Function):
    ...
    def forward(ctx, inputs: torch.Tensor):
        ...
        _backend.quaternion_conjugate(inputs.contiguous(), B, outputs)  # type: ignore # nopep8
        return outputs
